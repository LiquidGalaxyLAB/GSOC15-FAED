...
from faed_management.models import Incidence
...
def sync_kmls_to_galaxy(emergency=False):
    ...
    if not emergency:
        os.system("sshpass -p 'lqgalaxy' scp faed_management/static/kml/kmls.txt lg@" + read_file() + ":" + server_path)
        time.sleep(1)
    # print "sshpass -p 'lqgalaxy' scp " + file_path + " lg@" + read_file() + ":" + server_path
    os.system("sshpass -p 'lqgalaxy' scp " + file_path + " lg@" + read_file() + ":" + server_path)
...
def sync_kmls_file():
    ...
    for i in Kml.objects.all():
        file.write("http://" + str(ip_server)[0:(len(ip_server) - 1)] + ":8000/static/kml/" + i.name + "\n")
            # print "http://" + str(ip_server)[0:(len(ip_server) - 1)] + ":8000/static/kml/" + "manage" + str(i.id) + "\n"
        # file.write("http://" + str(ip_server)[0:(len(ip_server) - 1)] + ":8000/static/kml/" + i.name + "\n")
...
def get_server_ip():
    p = subprocess.Popen("ifconfig eth0 | grep 'inet:' | cut -d: -f2 | awk '{print $1}'", shell=True,
